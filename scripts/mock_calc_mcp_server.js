#!/usr/bin/env node
function writeResponse(requestId, result, error) {
  const payload = { jsonrpc: '2.0', id: requestId };
  if (error) {
    payload.error = error;
  } else {
...
function handleRequest(request) {
  ...
  if (method === 'tools/list') {
    writeResponse(requestId, {
      tools: [
        ...
        { name: 'divide', description: 'Divide a by b', inputSchema: { type: 'object', required: ['a', 'b'], properties: { a: { type: 'number' }, b: { type: 'number' } } } },
      ],
    });
    return;
  }
  if (method === 'tools/call') {
    ...
    writeResponse(requestId, {
      content: [{ type: 'text', text: `${name}(${a},${b})=${value}` }],
      value,
      isError: false,
    });
    return;
